# Midjourney Prompt Skills

## Core Structure
- **Subject**: Main focus (person, object, scene)
- **Style**: Artistic medium (photo, painting, illustration, 3D render)
- **Details**: Specific characteristics (lighting, colors, materials)
- **Parameters**: Technical settings (--ar, --v, --style, --no)

## Quick Tips
- Be specific but concise
- Use commas to separate concepts
- Combine unexpected elements for unique results
- Reference artists/styles for consistency
- Experiment with parameter combinations

## Useful Parameters
- `--ar 16:9` - Widescreen aspect ratio
- `--v 6.0` - Use latest model version
- `--style raw` - Less opinionated interpretation
- `--no` - Exclude elements (e.g., `--no text, blur`)
- `--s` - Stylize value (0-1000)

## Example Formula
"},"logprobs":null,"finish_reason":"length"}],"usage":{"prompt_tokens":19,"completion_tokens":200,"total_tokens":219,"prompt_tokens_details":{"cached_tokens":0},"prompt_cache_hit_tokens":0,"prompt_cache_miss_tokens":19},"system_fingerprint":"fp_eaab8d114b_prod0820_fp8_kvcache